...
namespace electrons {
    ...
    {
    public:
        ProducerTest( const edm::ParameterSet & );
    ...
    ProducerTest::ProducerTest( const ParameterSet &iConfig ):
        ...
        pfcandidateToken_( consumes<View<pat::PackedCandidate> >( iConfig.getParameter<InputTag> ( "pfCandidatesTag" ) ) )
    {
        ...
        produces<vector<electrons::Electron> >();
        // nontrigmva_ = 0;
    }

    void ProducerTest::produce( Event &evt, const EventSetup & )
    {
        //using namespace edm;
        ...
        std::unique_ptr<vector<electrons::Electron> > elecColl( new vector<electrons::Electron> );

        for( unsigned int elecIndex = 0; elecIndex < pelectrons->size(); elecIndex++ ) {
            Ptr<pat::Electron> pelec = pelectrons->ptrAt( elecIndex );
            electrons::Electron felec = electrons::Electron( *pelec );
...
typedef electrons::ProducerTest ElectronsProducerTest;
DEFINE_FWK_MODULE( ElectronsProducerTest );
